...
object Anagrams {
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y.toMap foldLeft x.toMap)(subtractTerm).filter(_._2 != 0).toList.sorted
  }

  def subtractTerm(x: Map[Char, Int], occurence: (Char, Int)): Map[Char, Int] = {
    val (ch, freq) = occurence
    x + (ch -> (x(ch) - freq))
  }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def occurrencesAnagrams(o: Occurrences): List[Sentence] = o match {
      case Nil => List(List())
      case h :: tail => {
        for {
          comb <- combinations(o)
          word <- dictionaryByOccurrences(comb)
          sentence <- occurrencesAnagrams(subtract(o, comb))
        } yield word :: sentence
      }
    }
