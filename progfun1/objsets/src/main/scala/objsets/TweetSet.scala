...
abstract class TweetSet {
  ...
  /**
    ...
    * and be implemented in the subclasses?
    */
  def mostRetweetedRec(t: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  override def mostRetweetedRec(currentTop: Tweet): Tweet = currentTop
  override def mostRetweeted: Tweet = throw new NoSuchElementException("Empty set has no elements")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweetedRec(currentTop: Tweet): Tweet = {
    if (elem.retweets > currentTop.retweets)
      this.remove(elem).mostRetweetedRec(elem)
    else
      this.remove(elem).mostRetweetedRec(currentTop)
  }
  override def mostRetweeted: Tweet = {
    mostRetweetedRec(elem)
  }
